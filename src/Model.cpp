...
glm::mat4 Model::Draw(Shader& shader, Camera& camera)
{	
	glm::mat4 mat;
	// Go over all meshes and draw each one
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
		mat = meshes[i].Mesh2::Draw(shader, camera, matricesMeshes[i]);
...
glm::mat4 Model::Draw(Shader& shader, Camera& camera, glm::mat4 trans, glm::mat4 rot, glm::mat4 sca) 
{
	glm::mat4 mat;
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
		mat = meshes[i].Mesh2::Draw_Transform(shader, camera, matricesMeshes[i], trans, rot, sca);
		//std::cout << glm::to_string(meshes[0].vertices[8].position) << "\n";
	}
	return mat;
}
...
std::vector<glm::mat4> Model::getMatrices(){
