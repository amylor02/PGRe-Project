...
void main()
{
    ...
    vec3 T = normalize(vec3(data_in[0].modelo * vec4(tangent, 0.0f)));
    vec3 B = normalize(vec3(data_in[0].modelo * vec4(bitangent*(-1), 0.0f)));
    vec3 N = normalize(vec3(data_in[0].modelo * vec4(cross(edge1, edge0), 0.0f)));
